...
At module level (`s.frame = none`) reading and writing globals is a plain
association-list update, and these two lemmas are all a proof needs. -/

theorem lookup_set_self {s : State} (hf : s.frame = Option.none) (x : String) (v : Value) :
    (s.setVar x v).lookupName x = some v :=
  State.lookupName_setVar_self s x v hf

theorem lookup_set_other {s : State} (hf : s.frame = Option.none) {x y : String} (hxy : x ≠ y)
